    ...
    return `<div class="thesis accordion-item">
                <h2 class="accordion-header" id="flush-${thesis.id}">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                        data-bs-target="#flush-collapse_${thesis.id}" aria-expanded="false"
                ...
                <div id="flush-collapse_${thesis.id}" class="accordion-collapse collapse" aria-labelledby="flush-${thesis.id}"
                    ...
                    <div class="accordion-body">
                        <i><p>${thesis.task}</p></i>
                        ${thesis.keys.length > 0 ? `<b>Kulcsszavak:</b><br><p>[${thesis.keys.join(", ")}]</p><br>` : ""}
    ...
    elements.forEach(el => {
        el.addEventListener("click", () => {
            ...
            if (text.innerHTML.length > 0) {
                let listeningBlock = el.parentElement.querySelector(".listening-modules");
                listeningBlock.innerHTML = readerControlPanelHTML();
            }
            setTimeout(() => {
                let visible = el.parentElement.querySelector(".accordion-collapse").classList.contains('show');
                let listeningBlock = el.parentElement.querySelector(".listening-modules");
